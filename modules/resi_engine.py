...
# Batas ciut otomatis (line height & font) saat produk banyak, biar tetap muat 1 label
LINE_HEIGHT_MM_MIN = 2.2
FONT_SIZE_MAX, FONT_SIZE_MIN = 7.0, 5.5
...
def build_product_lines_from_row(row: pd.Series, max_items: int = 10) -> Dict[str, List[Tuple[str, str]]]:
    ...
    for i in range(1, 11):
        ...
        if ("CBHB" in sku_upper) or ("CBGM" in sku_upper) or ("BOX" in sku_upper) or ("KARDUS" in sku_upper):
        ...
        elif any(k in sku_upper for k in ["TASBIH", "GLAS", "SPTL", "SHKR", "HADIAH", "BONUS", "TOPLES", "FLYER", "PANDUAN", "TOTEBAG", "CSO", "CRM"]):
            cat = "HADIAH"
            if "TASBIH" in sku_upper:
                ...
                name = "Bonus Panduan"
            elif "TOTEBAG" in sku_upper:
                name = "Hadiah Totebag"
            else:
                name = "Hadiah/Bonus"
            if sku_upper != name.upper():
                name = f"{name} [{sku_val}]"

        else:
            barang_keys = ["GMP", "GMS", "GP", "GM", "EC", "PB", "GN"]
...
def _sig_from_row_products(row: pd.Series) -> str:
    ...
    for i in range(1, 11):
        ...
        if ("CBHB" in sku_upper) or ("CBGM" in sku_upper) or ("BOX" in sku_upper) or ("KARDUS" in sku_upper):
            continue
        if any(k in sku_upper for k in ["TASBIH","GLAS","SPTL","SHKR","HADIAH","BONUS","TOPLES","FLYER","PANDUAN","TOTEBAG","CSO","CRM"]):
            continue
...
def export_pdf_a6_style_produk(pages: List[dict], scale_logo_bytes: Optional[bytes] = None) -> bytes:
    ...
    for i, p in enumerate(pages):
        ...
        y2 = panel_top_y - 6 * mm
        available_h = y2 - panel_bottom_y

        line_h_max, line_h_min = LINE_HEIGHT_MM * mm, LINE_HEIGHT_MM_MIN * mm
        n_rows = max(1, len(rows))
        needed_line_h = available_h / n_rows if n_rows > 1 else line_h_max
        line_h = min(line_h_max, max(line_h_min, needed_line_h))
        shrink = (line_h - line_h_min) / (line_h_max - line_h_min) if line_h_max > line_h_min else 1.0
        font_size = FONT_SIZE_MIN + shrink * (FONT_SIZE_MAX - FONT_SIZE_MIN)

        max_lines = max(1, int(available_h / line_h) + 1)
